...
def process(wav_name):
    ...
    if os.path.exists(wav_path) and "_mic2.flac" in wav_path:
        ...
        wavfile.write(
            save_path1, args.sr1, (wav1 * np.iinfo(np.int16).max).astype(np.int16)
        )
        wavfile.write(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_dir",
        type=str,
        default="/home/Datasets/lijingyi/data/vctk/wav48_silence_trimmed/",
        help="path to source dir",
    )
    parser.add_argument(
        "--out_dir1", type=str, default="./dataset/vctk-16k", help="path to target dir"
    )
    parser.add_argument(
        "--out_dir2", type=str, default="./dataset/vctk-22k", help="path to target dir"
    )
